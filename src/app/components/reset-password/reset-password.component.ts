...
export class ResetPasswordComponent implements OnInit {
  ...
  constructor(
    private restServ: ResetPassService,
  ...
  changePassword() {
    if (this.url.snapshot.params.hash) {
      ...
      this.values = new HttpParams().set(
        "newPassword",
        this.reset.value.password_validations.newPass
      );
      this.spinner.show();
      this.restServ.restorePassword(this.hash, this.values).subscribe(
        (res: any) => {
          this.toast.success(
      ...
      this.spinner.show();
      this.restServ.changePassword(this.hash, this.values).subscribe(
        (res: any) => {
          this.toast.success(
            "Se ha modificado su contraseña con éxito es necesario que vuelva a iniciar sesión ",
            "Éxito!"
          );
  ...
  verifyUrl(hashID: any) {
    ...
    this.restServ.verificarUrl(this.hash).subscribe(
      (res: any) => {
        this.isValid = true;
      },
      (error) => {
